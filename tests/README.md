# Test suite

Every fixture is synthesized in memory by `fixtures/build.py`. **No real binary
is fetched, committed, or scanned anywhere in this suite** — a test that needed
one would be a test nobody could rerun on a clean machine, and a repo full of
crypto-bearing executables trips antivirus on contributors' machines.

```
pytest                      # everything (includes a 200-mutant fuzz subset)
SHORSIGHTED_FUZZ=full pytest tests/test_fuzz.py   # the full 2,000-mutant sweep
pytest --cov                # with the coverage gate (>= 90% line, 80% branch)
pytest tests/test_merge.py  # one file
pytest -k corroborat        # by name
```

Two generated artefacts are committed and regenerate deliberately, never
automatically — the point of both is that a change has to be reviewed by a
person before it lands:

```
python -m tools.derive_constants   # signature tables in shorsighted/signatures/data/
python -m tests.regenerate_golden  # tests/golden/cng-sample.cbom.json
```

The evaluation corpus is a third such artefact, and the only one that needs a
compiler:

```
python -m corpus.build             # 104 binaries via zig cc, gitignored
python -m eval.run --print         # eval/report.md, committed
python -m eval.run --check         # fails if confidence.toml has drifted
```

## Traceability

Which test holds each acceptance criterion up (requirements §6).

| AC | Claim | Where it is checked |
|---|---|---|
| **AC-1** | CNG binary → AES, SHA-256, RSA with correct quantum levels and import evidence | `test_detector_imports.py::test_ac1_shape_reports_correct_quantum_levels`, `test_cli.py::test_a_synthetic_cng_binary_reports_its_algorithms` |
| **AC-2** | Statically linked binary with no crypto imports → AES and SHA-256 via constants | `test_detector_constants.py::test_a_binary_with_no_crypto_imports_still_reports_aes`, `test_cli.py::test_a_statically_linked_binary_is_detected` |
| **AC-3** | Embedded DER certificate → `certificate` asset with offset evidence | `test_detector_heuristics.py::test_a_der_certificate_is_found`, `::test_the_certificate_offset_points_at_the_structure` |
| **AC-4** | Mixed tree completes, .NET marked unsupported, malformed errored, exit 2 | `test_traits_and_walk.py::test_a_mixed_tree_scan_reports_every_category`, `test_cli.py::test_a_directory_with_a_broken_file_exits_two` |
| **AC-5** | Every emitted CBOM validates against the official CycloneDX 1.6 schema in CI | `test_output_cbom.py::test_every_document_shape_validates` (12 shapes × 2 modes), `::test_a_real_scan_produces_a_valid_cbom`, `test_cli.py::test_cli_json_validates_against_the_schema` |
| **AC-6** | A new algorithm added purely as signature data is detected with zero Python changes | `test_signatures.py::test_a_data_only_contribution_is_detected_end_to_end` — drops [`fixtures/sm4-contribution.toml`](fixtures/sm4-contribution.toml) into a copy of the shipped signature directory and asserts SM4 reaches the CBOM, with `::test_the_shipped_signatures_do_not_know_sm4` as its control. Plus the data path and its error messages, throughout that file |
| **AC-7** | Published evaluation report meeting the NFR-5 targets | [`eval/report.md`](../eval/report.md), regenerated by `python -m eval.run` against `corpus/`. Calibration arithmetic and corpus consistency: `test_eval.py`. Precision 1.000 on all three detectors; recall 0.797 including the defeat set |
| **NFR-2** | Survives malformed input without crash or hang; per-file timeout | `test_fuzz.py` — 200 mutants per PR across 9 mutation kinds, 2,000 nightly |
| **UI-1…3** | The report renders every file category, degrades on a foreign CBOM, and its numbers equal the document's | `test_output_report.py` |
| **UI-5…6** | Status and signature strings verbatim; no colour outside the frozen token table | `test_output_report.py::test_status_strings_are_the_enum_verbatim`, `::test_no_colour_outside_the_frozen_token_table` |
| **UI-7** | A hostile filename cannot escape into markup | `test_output_report.py::test_every_interpolated_field_is_escaped` (and a real hostile filename on filesystems that allow one) |
| **UI-8** | The clean-file appendix may drop names, never counts | `test_output_report.py::test_a_long_clean_list_collapses_but_keeps_its_count` |
| **AC-8** | `mypy --strict`, `ruff`, tests pass on Linux and Windows; wheel installs | `.github/workflows/ci.yml` — 8 matrix legs plus a clean-env wheel install |

## What the suite deliberately does not prove

- **Confidence numbers.** These tests do not check that `confidence.toml` is
  *right* — that is the corpus's job, and `python -m eval.run --check` is where
  it is enforced. What `test_eval.py` does check is the arithmetic that produces
  them: that a Wilson bound never reaches 1.0, that more samples earn more
  confidence, and that nothing ships outside the calibrated range. A bug there
  would be invisible, because the wrong numbers would still look plausible.
- **Real-world recall.** The corpus is *built*, not collected. Synthetic
  fixtures show a detector works on a table placed where the test put it; the
  corpus shows real compilers emit those tables in recognisable form. Neither
  shows what a shipping OpenSSL build looks like. See
  [`../LIMITATIONS.md`](../LIMITATIONS.md) §8.
- **That the report prints well.** Nothing in CI opens a browser. Page
  breaks, repeated headers, and background fills are eyeballed once per
  layout change against [`../docs/print-check.md`](../docs/print-check.md).
- **Packing thresholds.** `pe/traits.py` uses heuristic cutoffs with no ground
  truth behind them. The tests pin the behaviour at those values; they do not
  argue the values are right.
